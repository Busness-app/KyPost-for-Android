...
**Out of scope, deliberately:** porting OpenPGP to this app. An earlier plan called for it. It is
superseded and should not be revived without a conversation. The reasoning, in short: this device
pairs by QR and never learns the account password, the private key is wrapped under a key derived
from that password, and every workaround is worse — a per-device key breaks inbound mail (a sender
encrypts to whichever single key they discovered via WKD/Autocrypt), a Keystore-held key makes mail
recoverable without any user secret, and server-side re-encryption requires the server to hold the
key it would be protecting. **This device never holds the account's private key.**
...
| `protection` | Who holds the private key | Sending from this app |
|---|---|---|
...
`X-Kypost-Device-Id` / `X-Kypost-Device-Secret` headers. There is no mobile login and no session
cookie. Every endpoint below is `withMailAuth`, meaning a paired device is a first-class caller.

**Body format note:** the two 409s and the 200 are JSON. Every other error status returns a
**plain-text** body, not JSON. Do not run a JSON decoder over a 400/502/503 body and report
"malformed response" when it fails.

### `GET /api/pgp/bootstrap`

Call once per app start, before offering an encrypt toggle. Response 200:
...
`payloadEndpoint`, …) that exist for the browser. **Ignore them.** The only fields this app needs
...
The same endpoint this app already uses. Three fields are relevant here, all optional and all
defaulting to `false`:
...
```
Categorical. No retry from this device fixes it. Go to the webmail handoff below.
...
`server`-custody app asking `resolve` "does this recipient have a key" is refused every time. An
...
path is gated to `client`-custody accounts, which cannot send from this app at all — so **every**
pickup link this app causes to be sent is the server-readable kind. The confirmation UI must say
so honestly. This is the entire reason the opt-in exists; softening the copy defeats it.

## Required behavior

1. **On app start**, `GET /api/pgp/bootstrap`. Cache `hasIdentity` and `protection` for the session.
...
5. **On 409 with `keylessRecipients`**: show a confirmation naming those exact addresses, stating
   ...
   server for up to seven days. Default the dialog to cancel. On confirm, re-send the **byte-identical
   request** with `allowPickupFallback: true`. Do not re-run the preflight, do not rebuild the
   message, do not re-encode attachments — rebuild risks a subtly different message.
6. **On 409 with `clientSideNeeded`**: `POST /api/mail/draft` with the same body (minus the PGP
   fields) to save the composed message, then hand `/read?mailbox=Drafts` to the **system** as a
   normal https intent so an installed PWA or the user's browser handles it. Explain why: the key
   is held only in the browser. Never open this in an in-app WebView — it shares no session and
...
- Any OpenPGP implementation, key generation, or key unwrapping in this app. See Scope.
...
- Any call to `POST /api/mail/send-pgp`. That endpoint takes a pre-built ciphertext delivery from a
  client that did its own encryption; this app never has one.
...
## Current state of this repo (`kypost-android`)

Already implemented, do not redo:

- Read-side PGP message state — `app/src/main/java/com/urlxl/mail/pgp/PgpMessageState.kt` — reads
  `pgpEncrypted`, `pgpSigned`, `pgpVerified`, `pgpSignerFingerprint`, `pgpDecryptError` off the
  inbox row and renders the "encrypted mail this app can't read" explanation.
- The webmail deep link for **reading** a message, in `EmailDetailActivity.kt`.
- Draft save (`MailSource.saveDraft` → `POST /api/mail/draft`).
- The `clientSideNeeded` 409, at `RelayMailSource.kt:23` (the `CLIENT_SIDE_NEEDED_MARKER` constant)
  and its branch in `mapErrorCode` around `RelayMailSource.kt:300`.
- `warning` parsing — `RelaySendResponseDto` at `RelayModels.kt:141` already carries it into
  `MailSendOutcome`. **Verify the UI actually displays it**; the web client only started doing so
  in the last commit of the server branch, and this app may have inherited the same gap.

To add:

- `encrypt`, `sign`, `allowPickupFallback` on `RelayMailRequestDto` (`RelayModels.kt:107`) and
  whatever carries them from `MailDraft` through `toWireDto()`.
- A second 409 shape. `mapErrorCode` currently tests for the `clientSideNeeded` marker and falls
  through to `BadRequest(rawBody)` — which would show the user raw JSON for the new 409. This is
  not a live regression today only because the app never sets `encrypt`, so it never reaches the
  gate. Add a `MailOutcome` variant carrying `keylessRecipients`, and parse the body properly
  rather than substring-matching it (the existing marker check is a deliberate substring test on a
  body that may be a plain-text error; the new one needs the address list, so decode it).
- The preflight call and its DTOs.
- The compose-side confirmation dialog and re-send.

Tests live in `app/src/test/java/com/urlxl/mail/mail/RelayMailSourceTest.kt` and use the existing
fake-call-factory pattern. Cover at minimum: both 409 shapes mapping to distinct outcomes; a 409
body with neither field falling back to a generic error; the re-send carrying
