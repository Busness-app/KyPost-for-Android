...
- Recovering a credential is NOT unpairing. `PushRepository.resetPairingCredential` drops the pairing proof and TLS pin while keeping mail, contacts and keys; `clearPairing`/`unpairDevice` stay the destructive pair, for a deliberate unpair and for `attemptPairing`'s account-replacement branch. A rotated certificate or a stranded device secret must route to the former — "Reconnect to server" on the pairing screen — never the latter. `resetPairingCredential` WRITES the `ResidentAccount` marker (subscriber id + server URL) from the pairing it is clearing — not merely spares an existing one, because a pairing saved by any build older than the marker has none, and that install is the likeliest to reconnect — and `attemptPairing` checks the next pairing against it; without it a reconnect looked like a never-paired device, so any account's QR paired over the previous account's mail, contacts and keys with no purge and no warning. `clearPairing`, unpair and the wipe clear it.
