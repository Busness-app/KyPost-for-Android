...
class SecurityGraph(context: Context) {
     ...
     * function ([com.urlxl.mail.push.PushRepository.savePairing]) — and each one builds its own
     * `MasterKey` and `EncryptedSharedPreferences`, meaning a Keystore access and a Tink keyset
     * load apiece. [com.urlxl.mail.push.SecurePairingStore]'s KDoc already diagnosed exactly this
